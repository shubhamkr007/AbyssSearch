...
def create_app() -> FastAPI:
    app = FastAPI(
        ...
        description="S14 optional cross-encoder second stage over hybrid search results.",
        version="0.1.0",
        lifespan=lifespan,
    )
    settings = get_settings()
    app.state.settings = settings
    # Eager wiring so TestClient works without entering the lifespan context.
